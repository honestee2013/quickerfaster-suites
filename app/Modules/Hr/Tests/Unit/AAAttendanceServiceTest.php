...
class AAAttendanceServiceTest extends TestCase
{
    use RefreshDatabase;
...
/*class AttendanceServiceTest extends TestCase
{
    ...
    {
        $user = User::factory()->create(['hourly_rate' => 20]);
        $schedule = RoleSchedule::factory()->create([
        ...
        $attendance = DailyAttendance::factory()->create([
            'user_id' => $user->id,
        ...
        $this->attendanceService->checkOutAndCalculateDailyEarnings($attendance->fresh());

        $this->assertEquals(160.00, $attendance->fresh()->daily_earnings);
    }

    public function it_calculates_pay_with_overtime()
    {
        $user = User::factory()->create(['hourly_rate' => 20]);
        $schedule = RoleSchedule::factory()->create([
            'role_id' => $user->role_id,
        ...
        $attendance = DailyAttendance::factory()->create([
            'user_id' => $user->id,
            'check_in_time' => Carbon::today()->setTime(9, 0),
            'check_out_time' => Carbon::today()->setTime(19, 0), // 10 hours
            'date' => today()
        ]);
        ...
        $this->assertEquals(220.00, $attendance->fresh()->daily_earnings);
    }

    public function it_applies_break_rules_before_calculating_pay()
    {
        $user = User::factory()->create(['hourly_rate' => 20]);

        $breakRule = BreakRule::factory()->create([
            'min_shift_minutes' => 300, // 5 hours
            'break_interval_minutes' => 240,
        ...
        $attendance = DailyAttendance::factory()->create([
            'user_id' => $user->id,
            'check_in_time' => Carbon::today()->setTime(9, 0),
            'check_out_time' => Carbon::today()->setTime(18, 0), // 9 hours
            'date' => today()
        ]);
        ...
        // 9 hours = 540 minutes
        // Break rule applies: 1 break (30 min) deducted => 8.5 hours
        // 8 regular hours + 0.5 overtime
        $expectedPay = (8 * 20) + (0.5 * 20 * 1.5);
        $this->assertEquals($expectedPay, $attendance->fresh()->daily_earnings);
    }
}*/
